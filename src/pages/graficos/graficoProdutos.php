...
if (isset($_POST['submit'])) {
    ...
    /* FILTRO SELECIONADO */
    $grafico = mysqli_query($conexao, "SELECT nomeProduto, SUM(qtdProduto) totalProdutos, SUM(qtdProduto)*valorProduto totalGerado 
    FROM pedidos WHERE dataVenda BETWEEN ('2022-$mes-01') AND ('2022-$mes-31') GROUP BY nomeProduto  ORDER BY SUM(qtdProduto) DESC LIMIT 5");
    $grafico = mysqli_fetch_all($grafico);

    if ($mes == 00) {
        $grafico = mysqli_query($conexao, 'SELECT nomeProduto, SUM(qtdProduto) totalProdutos, SUM(qtdProduto)*valorProduto totalGerado 
    FROM pedidos where month(dataVenda) = month(now()) and year(dataVenda) = year(now())
    GROUP BY nomeProduto  ORDER BY SUM(qtdProduto) DESC LIMIT 5');
        $grafico = mysqli_fetch_all($grafico);
    }
} else {
        ...
        <div class="home-content" style="height: 100%;">
            ...
            <div class="row align-items-center justify-content-center" style="height: 100%;">
                <div class="col-md-9" style="height: 100%;">
                    <div class="row">
                        <form action="../graficos/graficoProdutos.php" method="POST">
                    ...
                    <div style="height:80%;" id="container">
                        <script>
                            anychart.onDocumentReady(function() {
                                ...
                                // set the chart title
                                chart.title("Produtos mais vendidos no mês");
